        ...
        try:
            ...
            with open(csv_file, 'r') as f:
                self.cursor.copy_expert(
                    "COPY oop_data_iogithub (title, location, posted, role, job_details, link)"
...
def main():
    ...
    create_table_sql = '''
        CREATE TABLE IF NOT EXISTS oop_data_iogithub(
            id INTEGER GENERATED ALWAYS AS IDENTITY,
            title varchar(100) UNIQUE,
            location varchar(100),
            posted date,
            role varchar(100),
            job_details varchar(100),
            link varchar(100)
        );
    '''
    ...
    db.insert_csv('oop_data_iogithub', '../oop_data_iogithub.csv')
    print('Fetched data:')
    # Fetch and print all data from the jobs table
    for row in db.fetch_all('SELECT * FROM oop_data_iogithub;'):
        print(row)
